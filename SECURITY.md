...
**2026-08-10 に実際に起きた。** 2026-08-02 に「本番環境」に設定したと記録してあったが、
トークンは 7 日で切れた。つまりその設定は効いていなかったか、あとで戻っていた。

| 出来事 | 日時 (UTC) | 発行からの経過 |
|---|---|---|
| `GMAIL_CREDENTIALS` を設定 | 2026-08-02 17:39 | — |
| 最後の成功 | 2026-08-08 09:02 | 5 日 15 時間 |
| 最初の失敗 (`invalid_grant`) | 2026-08-10 00:15 | **7 日 7 時間** |

**再発行の前に公開ステータスを見ること。** 同じステータスのまま出し直したトークンは
翌週また死に、何も分からないまま 1 週間を捨てる。エラーメッセージ自身がこれを言う
ようにしてある (`infra/gmail/authfail.go`) — API は
`invalid_grant: "Token has been expired or revoked."` しか言わず、
expired と revoked のどちらなのかも教えない。

「本番環境」なのに切れたなら失効させられた側で、
myaccount.google.com/permissions での手動取り消しか Google アカウント側の変更を疑う。
