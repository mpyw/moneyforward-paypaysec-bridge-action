...
| 原因 | 見分け方 |
|---|---|
| Google アカウントのパスワード変更 | **Gmail スコープを持つトークンは全部無効になる。** 心当たりがあるかは本人しか知らない |
| 手動での連携解除 | myaccount.google.com/permissions から**アプリが消える** |
| OAuth クライアントの削除・シークレットのローテート | Google Cloud の Clients を見る |
| 同一クライアントの発行数上限超過 | 古いものから黙って無効化される。数十回発行していなければ該当しない |
...
**まず permissions ページを見る**のが一番安い。アプリが残っていれば解除ではないので、
パスワード変更かクライアント側を疑う。エラーメッセージ自身がこの一覧を出す
