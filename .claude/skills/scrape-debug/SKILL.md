...
- **非同期ロード中は全項目 0円 を表示する。** 3 ルート照合を全部通る
- **クリックに由来する状態は，データが古いうちから正しく見える。** 投資信託のタブで
  実測: `actived` も `menu_1_mini` も **8ms** で切り替わり，数字は **1028ms** で変わる
- したがって「値が変化しなくなった」は「新しいデータが来た」ではない
- **この 1 件は待つのをやめて解決した。** 投資信託は `investapi` がページの叩く
  エンドポイントを直接叩くので，観測すべき状態がない。同じ形（1 URL に複数ビュー）に
  ぶつかったら，待ち方を改良する前に **API を探す**
