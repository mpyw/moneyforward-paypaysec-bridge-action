...
- **タブ切替は「クリック同期」と「データ到着」が別。** 実測で `actived` も
  ...
  「値が変化しなくなった」＝ settle は**前のタブの値でも成立する**ので、これだけで
  読むと保有 2 銘柄のカテゴリを 0 件と読む。MutationObserver ＋ **10 秒の下限**で待つ。
  保証しているのは下限のほう — Observer だけだと初回ロードの残りで条件を満たす。
  **5 秒では足りなかった**（ホストランナーで再発）。待ちの代償は 1 日 10 秒、
  待たない代償は実在する銘柄の削除
- **内側の待ちの合計は外側の deadline より小さく保つ。** `targetTimeout` が 30 秒の
  ままだと settle 20 + repaint 25 + settle 20 を包めず、外側が先に切れて
